      ...
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 pt-20 pb-24 sm:pt-28 sm:pb-32">
        ...
        <div className="flex items-center gap-2.5 mb-10 fade-up-1">
          <div className="bg-primary p-2 rounded-xl">
            <HardHat size={22} className="text-dark" />
          </div>
          <span className="font-display text-xl text-white tracking-tight">BuildSafe</span>
        </div>

        {/* Headline */}
        <div className="max-w-3xl">
          <div className="fade-up-1 inline-flex items-center gap-2 bg-primary/10 border border-primary/20 rounded-full px-3 py-1 mb-6">
            <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />
            <span className="text-[11px] font-mono text-primary tracking-wide uppercase">
              Blockchain-Powered Wage Protection
            </span>
          </div>

          <h1 className="fade-up-2 font-display text-4xl sm:text-5xl md:text-6xl text-white leading-[1.08] tracking-tight mb-6">
            No more{" "}
            <span className="text-primary">wage theft</span>{" "}
          ...
          <div className="fade-up-4 flex flex-wrap gap-3">
            <button
              onClick={onLogin}
              className="flex items-center gap-2 bg-primary text-dark font-display text-sm px-6 py-3.5 rounded-xl hover:bg-primaryDark hover:scale-[1.02] active:scale-[0.97] transition-all duration-150 shadow-lg shadow-primary/20"
            >
...
function ProblemSnapshot() {
  ...
  return (
    <section className="bg-surface2 py-16 sm:py-20">
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        <div className="flex items-center gap-2 mb-2">
          <AlertTriangle size={15} className="text-danger" />
          <p className="font-mono text-[11px] text-danger uppercase tracking-widest">The Problem</p>
        </div>
        <h2 className="font-display text-2xl sm:text-3xl text-white mb-10 max-w-xl">
...
function HowItWorks() {
  ...
  return (
    <section id="how-it-works" className="bg-dark py-16 sm:py-20">
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        <div className="flex items-center gap-2 mb-2">
          <Link2 size={14} className="text-primary" />
          <p className="font-mono text-[11px] text-primary uppercase tracking-widest">How it works</p>
        </div>
        <h2 className="font-display text-2xl sm:text-3xl text-white mb-12 max-w-xl">
          Four steps, zero trust required.
        </h2>

        {/* Step strip — horizontal scroll on mobile */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-surface rounded-2xl overflow-hidden border border-border">
          {steps.map((s, i) => (
            <div key={i} className="bg-surface2 p-5 sm:p-6 relative group hover:bg-surface3 transition-colors duration-200">
              {/* Connector arrow (desktop only) */}
              {i < steps.length - 1 && (
                <ChevronRight
                  size={14}
                  className="hidden lg:block absolute -right-2 top-1/2 -translate-y-1/2 text-textMuted/40 z-10"
                />
              )}
              <div className="flex items-center gap-2 mb-4">
                <span className="font-mono text-[10px] text-textMuted">{s.step}</span>
              ...
              <h3 className="font-display text-sm text-white mb-1.5">{s.title}</h3>
              <p className="text-[11px] text-textMuted leading-relaxed">{s.desc}</p>
            </div>
          ))}
        </div>

        {/* Security callout */}
        <div className="mt-6 flex flex-wrap items-center gap-4 text-[11px] font-mono text-textMuted">
          {["SHA-256 hash chaining", "GPS + timestamp proof", "ArmorIQ biometric session", "ArmorPay policy gate"].map((f) => (
            <span key={f} className="flex items-center gap-1.5">
              <Lock size={10} className="text-primary" /> {f}
            </span>
          ))}
        </div>
      </div>
...
function LiveStats({ stats }) {
  const indicators = [
    ...
    },
    {
      ...
      icon: <ShieldCheck size={18} className={stats.valid === false ? "text-danger" : "text-primary"} />,
      color: stats.valid === false ? "border-danger/30" : "border-primary/30",
    },
    {
  ...
  return (
    <section className="bg-surface2 py-16 sm:py-20">
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        <div className="flex items-center gap-2 mb-2">
          <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />
          <p className="font-mono text-[11px] text-primary uppercase tracking-widest">Live ledger — real data</p>
        </div>
        <h2 className="font-display text-2xl sm:text-3xl text-white mb-2 max-w-xl">
          These numbers come directly from the blockchain.
        </h2>
        <p className="text-sm text-textMuted mb-10 max-w-lg font-mono">
        ...
        <p className="text-[10px] font-mono text-textMuted/50 mt-5 text-center">
...
function RoleCards({ onLogin }) {
  const roles = [
    {
      icon: <HardHat size={28} className="text-dark" />,
      bg: "bg-primary",
      badge: "primary",
      role: "Worker",
    ...
    <section className="bg-dark py-16 sm:py-20">
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        <div className="flex items-center gap-2 mb-2">
          ...
          <p className="font-mono text-[11px] text-textMuted uppercase tracking-widest">Who it's for</p>
        </div>
        <h2 className="font-display text-2xl sm:text-3xl text-white mb-10 max-w-xl">
          ...
          {roles.map((r) => (
            ...
            >
              ...
              <div className={`${r.bg} p-5 flex items-center gap-3`}>
                ...
                <div>
                  <p className={`font-display text-base leading-none ${r.role === 'Worker' ? 'text-dark' : 'text-white'}`}>{r.role}</p>
                  <p className={`text-[10px] font-mono mt-0.5 ${r.role === 'Worker' ? 'text-dark/60' : 'text-white/60'}`}>{r.email}</p>
                </div>
              </div>

              <div className="p-5 flex-1 flex flex-col">
                <p className="text-sm text-white leading-snug mb-4">{r.tagline}</p>
        ...
        <div className="mt-8 bg-surface border border-border rounded-xl p-4 flex flex-wrap items-center gap-3">
          <ShieldCheck size={14} className="text-primary shrink-0" />
          <p className="text-[11px] font-mono text-textMuted">
            <span className="text-white font-semibold">Demo mode</span> — all accounts use password{" "}
            <span className="bg-surface2 px-1.5 py-0.5 rounded text-white font-bold">demo123</span>.
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <div className="bg-primary p-1 rounded-lg">
            <HardHat size={14} className="text-dark" />
          </div>
          <span className="font-display text-sm text-white">BuildSafe</span>
        </div>
        <p className="text-[10px] font-mono text-textMuted text-center">
          Hash-chained ledger · ArmorPay policy-gated payouts · ArmorIQ biometric sessions
        </p>
        <p className="text-[10px] font-mono text-textMuted/40">Finale prototype · June 2026</p>
      </div>
    </footer>
  );
}
